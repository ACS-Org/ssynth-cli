...
export async function streamWsLogs(jobId, client, json) {
  ...
  await new Promise((resolve, reject) => {
    ...
    function finish(status) {
      ...
      if (status) {
        console.error();
        console.error(`${chalk.bold("Job finished:")} ${colorizeStatus(status)}`);
      }
      ws.close();
      resolve();
    }

    // Fallback: poll REST API for job status in case the WS never
    // delivers a status message (zombie connection, server bug, etc.)
    ws.addEventListener("open", () => {
      statusPoll = setInterval(async () => {
        try {
          ...
          // ignore poll errors; WS is primary
        }
      }, STATUS_POLL_MS);
    });
