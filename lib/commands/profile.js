...
function buildToolArgs(opts) {
  const toolArgs = {};
  if (opts.yosysPasses) {
    toolArgs.yosys = { extra_passes: opts.yosysPasses.split(",").map((s) => s.trim()) };
  }
  if (opts.nextpnrFlags) {
    toolArgs.nextpnr = { extra_flags: opts.nextpnrFlags.split(",").map((s) => s.trim()) };
  }
  if (opts.bitstreamFlags) {
    toolArgs.bitstream = { extra_flags: opts.bitstreamFlags.split(",").map((s) => s.trim()) };
  }
