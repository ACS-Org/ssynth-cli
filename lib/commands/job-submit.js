...
export async function jobSubmitCommand(sourcePath, opts, client, config, json) {
  ...
  const profileId = opts.profile || hwbuild?.profile || undefined;
  // When a profile is specified, target_id is optional (API resolves from profile)
  let targetId;
  if (opts.target || (!profileId && hwbuild?.target)) {
    targetId = await resolveTarget(opts.target, hwbuild, client);
  } else if (!profileId) {
    targetId = await resolveTarget(undefined, hwbuild, client);
  }
  ...
  const req = buildJobRequest(opts, hwbuild, targetId, topModule, sourceKey, profileId);
...
function buildJobRequest(opts, hwbuild, targetId, topModule, sourceKey, profileId) {
  ...
  if (targetId) { req.target_id = targetId; }
  ...
  // Tool args from hwbuild.yml
  const toolArgs = hwbuild?.tool_args;
  if (toolArgs) { req.tool_args = toolArgs; }
