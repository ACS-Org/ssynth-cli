...
async function loginApiKey(opts, config, apiUrl) {
  ...
  if (tenants.length > 0) {
    console.error(`  Tenant: ${tenants[0].id}`);
  }
}
...
async function loginDev(opts, config, apiUrl) {
  ...
  const expires = login.expires_at
    ? new Date(login.expires_at).toISOString()
    : new Date(Date.now() + (login.expires_in || 3600) * 1000).toISOString();
  ...
  console.error(`  User:   ${login.user.id}`);
