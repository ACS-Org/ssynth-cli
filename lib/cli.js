...
export function run() {
  ...
  jobCmd
    ...
    .option("--profile <id>", "Build profile ID")
  ...
  const profileCmd = program
    .command("profile")
    .description("Manage build profiles");

  profileCmd
    .command("list")
    .description("List build profiles for a project")
    .requiredOption("--project <id>", "Project ID")
    .action(wrapAction(async (opts) => {
      const { client, json } = authedContext(program);
      await profileListCommand(opts.project, client, json);
    }));

  profileCmd
    .command("create")
    .description("Create a new build profile")
    .requiredOption("--project <id>", "Project ID")
    .requiredOption("--name <name>", "Profile name")
    .requiredOption("--target <id>", "Target ID")
    .option("--description <text>", "Profile description")
    .option("--yosys-passes <list>", "Comma-separated yosys passes (e.g. opt,abc9)")
    .option("--nextpnr-flags <list>", "Comma-separated nextpnr flags (e.g. --router=router2)")
    .option("--bitstream-flags <list>", "Comma-separated bitstream flags (e.g. --compress)")
    .action(wrapAction(async (opts) => {
      const { client, json } = authedContext(program);
      await profileCreateCommand(opts.project, opts, client, json);
    }));

  profileCmd
    .command("get <id>")
    .description("Get build profile details")
    .action(wrapAction(async (id) => {
      const { client, json } = authedContext(program);
      await profileGetCommand(id, client, json);
    }));

  profileCmd
    .command("update <id>")
    .description("Update a build profile")
    .option("--name <name>", "New profile name")
    .option("--description <text>", "New description")
    .option("--target <id>", "New target ID")
    .option("--yosys-passes <list>", "Comma-separated yosys passes")
    .option("--nextpnr-flags <list>", "Comma-separated nextpnr flags")
    .option("--bitstream-flags <list>", "Comma-separated bitstream flags")
    .action(wrapAction(async (id, opts) => {
      const { client, json } = authedContext(program);
      await profileUpdateCommand(id, opts, client, json);
    }));

  profileCmd
    .command("delete <id>")
    .description("Delete a build profile")
    .action(wrapAction(async (id) => {
      const { client, json } = authedContext(program);
      await profileDeleteCommand(id, client, json);
    }));
